    ...
    *,
    ...
    if use_type_alias:
        if data_model_type == DataModelType.PydanticBaseModel:
            root_model = (
                rootmodel.PydanticV1TypeAliasBackportRootModel
                if target_python_version == PythonVersion.PY_39
                else rootmodel.PydanticV1TypeAliasRootModel
            )
        elif target_python_version in {PythonVersion.PY_312, PythonVersion.PY_313, PythonVersion.PY_314}:
            root_model = rootmodel.NativeTypeAliasRootModel
        elif data_model_type == DataModelType.PydanticV2BaseModel:
            root_model = rootmodel.TypeAliasTypeRootModel
        else:
            root_model = (
                rootmodel.TypeAliasBackportRootModel
                if target_python_version == PythonVersion.PY_39
                else rootmodel.TypeAliasRootModel
            )
    elif data_model_type == DataModelType.PydanticBaseModel:
    ...
    if data_model_type == DataModelType.MsgspecStruct:
        return DataModelSet(
            ...
            root_model=root_model if use_type_alias else msgspec.RootModel,
