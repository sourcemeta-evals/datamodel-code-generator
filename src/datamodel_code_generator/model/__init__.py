...
def get_data_model_types(
    ...
    if data_model_type == DataModelType.PydanticBaseModel:
        ...
        if use_type_alias:
            root_model = (
                pydantic.CustomRootTypeTypeAliasBackport
                if target_python_version == PythonVersion.PY_39
                else pydantic.CustomRootTypeTypeAlias
            )
        return DataModelSet(
    ...
    if data_model_type == DataModelType.PydanticV2BaseModel:
        ...
        if use_type_alias:
            root_model = (
                pydantic_v2.RootModelNativeTypeAlias
                if target_python_version.has_type_statement
                else pydantic_v2.RootModelTypeAliasType
            )
        return DataModelSet(
    ...
    if data_model_type == DataModelType.DataclassesDataclass:
        root_model = (
            rootmodel.NativeTypeAliasRootModel
            if use_type_alias and target_python_version.has_type_statement
            else rootmodel.TypeAliasRootModelBackport
            if use_type_alias and target_python_version == PythonVersion.PY_39
