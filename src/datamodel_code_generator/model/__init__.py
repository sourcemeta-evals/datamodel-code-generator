...
def _get_type_alias_root_model(
    ...
    """Return the appropriate type alias model class based on
    the output model type and target Python version.

    Version matrix:
    - Python 3.12+: native ``type`` statement (all output types)
    - Pydantic v2 + Python 3.9-3.11: ``TypeAliasType`` from ``typing_extensions``
    - Pydantic v1 + all Python versions: ``TypeAlias`` annotation
    - Non-Pydantic output types + Python 3.9-3.11: ``TypeAlias`` annotation
    """
    from datamodel_code_generator import DataModelType  # noqa: PLC0415

    from .type_alias import TypeAliasModel, TypeAliasTypeModel, TypeStatementModel  # noqa: PLC0415
