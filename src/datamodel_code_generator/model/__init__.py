...
def _get_type_alias_model(
    data_model_type: DataModelType,
    target_python_version: PythonVersion,
) -> type[DataModel]:
    """Return the appropriate type alias model class based on output type and Python version."""
    from datamodel_code_generator import DataModelType  # noqa: PLC0415

    from .type_alias import (  # noqa: PLC0415
        TypeAliasModel,
        TypeAliasModelNativeType,
        TypeAliasModelTypeAliasType,
    )

    if target_python_version.has_type_statement:
        # Python 3.12+: use native `type X = ...` for all output types
        return TypeAliasModelNativeType

    if data_model_type == DataModelType.PydanticV2BaseModel:
        # Pydantic v2 + Python <3.12: use TypeAliasType from typing_extensions
        return TypeAliasModelTypeAliasType

    # Pydantic v1 / non-Pydantic + Python <3.12: use TypeAlias annotation
...
def get_data_model_types(
    ...
    *,
    ...
    type_alias_root: type[DataModel] | None = None
    if use_type_alias:
        type_alias_root = _get_type_alias_model(data_model_type, target_python_version)

    if data_model_type == DataModelType.PydanticBaseModel:
        return DataModelSet(
            data_model=pydantic.BaseModel,
            root_model=type_alias_root or pydantic.CustomRootType,
    ...
    if data_model_type == DataModelType.PydanticV2BaseModel:
        return DataModelSet(
            ...
            root_model=type_alias_root or pydantic_v2.RootModel,
    ...
    if data_model_type == DataModelType.DataclassesDataclass:
        return DataModelSet(
            ...
            root_model=type_alias_root or rootmodel.RootModel,
    ...
    if data_model_type == DataModelType.MsgspecStruct:
        return DataModelSet(
            ...
            root_model=type_alias_root or msgspec.RootModel,
