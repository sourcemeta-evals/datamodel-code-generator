...
def get_data_model_types(
    ...
    *,
    ...
    from .type_alias import create_type_alias_class  # noqa: PLC0415
    from .types import DataTypeManager  # noqa: PLC0415

    if data_model_type == DataModelType.PydanticBaseModel:
        root_model: type[DataModel]
        if use_type_alias:
            root_model = create_type_alias_class(target_python_version, is_pydantic_v2=False)
        else:
    ...
    if data_model_type == DataModelType.PydanticV2BaseModel:
        if use_type_alias:
            root_model = create_type_alias_class(target_python_version, is_pydantic_v2=True)
        else:
