...
def _get_type_alias_model(
    ...
    from .type_alias import TypeAliasAnnotation, TypeAliasTypeModel, TypeStatement  # noqa: PLC0415

    if target_python_version.has_type_statement:
        # Python 3.12+: use native `type` statement for all model types
    ...
    # Pydantic v1, dataclass, TypedDict, msgspec on Python 3.9-3.11:
    # use TypeAlias annotation from typing_extensions
...
def get_data_model_types(
    ...
    *,
    ...
    type_alias_model: type[DataModel] | None = (
        _get_type_alias_model(data_model_type, target_python_version) if use_type_alias else None
    )

    if data_model_type == DataModelType.PydanticBaseModel:
        return DataModelSet(
            data_model=pydantic.BaseModel,
            root_model=type_alias_model or pydantic.CustomRootType,
    ...
    if data_model_type == DataModelType.PydanticV2BaseModel:
        return DataModelSet(
            ...
            root_model=type_alias_model or pydantic_v2.RootModel,
    ...
    if data_model_type == DataModelType.DataclassesDataclass:
        return DataModelSet(
            ...
            root_model=type_alias_model or rootmodel.RootModel,
    ...
    if data_model_type == DataModelType.MsgspecStruct:
        return DataModelSet(
            ...
            root_model=type_alias_model or msgspec.RootModel,
