...
def _get_type_alias_model(
    ...
    if target_python_version.has_type_statement:
        return type_alias.NativeTypeAlias
    if data_model_type == DataModelType.PydanticV2BaseModel:
        return type_alias.TypeAliasTypeModel
    if target_python_version.has_typing_type_alias:
        return type_alias.TypeAlias
    return type_alias.TypeAliasBackport
...
def get_data_model_types(
    ...
    if data_model_type == DataModelType.PydanticBaseModel:
        root_model: type[DataModel]
        if use_type_alias:
            root_model = _get_type_alias_model(data_model_type, target_python_version)
        else:
