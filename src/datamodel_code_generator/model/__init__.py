...
def get_data_model_types(
    ...
    def create_type_alias_model_class(use_pydantic_v2: bool) -> type[DataModel]:
        class BoundTypeAliasModel(TypeAliasModel):
        ...
        return BoundTypeAliasModel

    if data_model_type == DataModelType.PydanticBaseModel:
        root_model: type[DataModel]
        if use_type_alias:
            root_model = create_type_alias_model_class(use_pydantic_v2=False)
        else:
    ...
    if data_model_type == DataModelType.PydanticV2BaseModel:
        if use_type_alias:
            root_model = create_type_alias_model_class(use_pydantic_v2=True)
        else:
