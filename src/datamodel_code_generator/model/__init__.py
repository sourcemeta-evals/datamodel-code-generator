...
def get_data_model_types(
    ...
    from . import dataclass, msgspec, pydantic, pydantic_v2, rootmodel, type_alias, typed_dict  # noqa: PLC0415
    from .types import DataTypeManager  # noqa: PLC0415

    py_312_or_later = target_python_version in {
        PythonVersion.PY_312,
        PythonVersion.PY_313,
        PythonVersion.PY_314,
    }
    type_alias_root_model = (
        type_alias.RootTypeAliasBackport if target_python_version == PythonVersion.PY_39 else type_alias.RootTypeAlias
    )

    if data_model_type == DataModelType.PydanticBaseModel:
        return DataModelSet(
            data_model=pydantic.BaseModel,
            root_model=type_alias_root_model if use_type_alias else pydantic.CustomRootType,
    ...
    if data_model_type == DataModelType.PydanticV2BaseModel:
        return DataModelSet(
            ...
            root_model=(
                type_alias.RootNativeTypeAlias
                if use_type_alias and py_312_or_later
                else type_alias.RootTypeAliasType
                if use_type_alias
    ...
    if data_model_type == DataModelType.DataclassesDataclass:
        return DataModelSet(
            ...
            root_model=(type_alias.RootNativeTypeAlias if use_type_alias and py_312_or_later else type_alias_root_model)
            if use_type_alias
