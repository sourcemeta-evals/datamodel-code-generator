...
class JsonSchemaObject(BaseModel):
    ...
    def is_const_enum(self) -> bool:
        """Check if oneOf/anyOf contains only const values, forming an enum."""
        items = self.oneOf or self.anyOf
        if not items:
            return False
        return all("const" in item.extras for item in items)
...
class JsonSchemaParser(Parser):
    ...
    def _build_enum_from_const(self, obj: JsonSchemaObject) -> JsonSchemaObject:
        """Build an enum-equivalent schema from oneOf/anyOf with const values."""
        items = obj.oneOf or obj.anyOf
        consts = [item.extras["const"] for item in items]
        titles = [item.title for item in items if item.title]

        enum_dict: dict[str, Any] = {"enum": consts}
        if obj.type:
            enum_dict["type"] = obj.type
        elif all(isinstance(c, str) for c in consts):
            enum_dict["type"] = "string"
        elif all(isinstance(c, int) and not isinstance(c, bool) for c in consts):
            enum_dict["type"] = "integer"
        elif all(isinstance(c, (int, float)) and not isinstance(c, bool) for c in consts):
            enum_dict["type"] = "number"
        if obj.title:
            enum_dict["title"] = obj.title
        if obj.description:
            enum_dict["description"] = obj.description
        if obj.has_default:
            enum_dict["default"] = obj.default
        if len(titles) == len(consts):
            enum_dict["x-enum-varnames"] = titles

        return self.SCHEMA_OBJECT_TYPE.parse_obj(enum_dict)
    ...
    def parse_item(  # noqa: PLR0911, PLR0912
        ...
        if item.anyOf or item.oneOf:
            if item.is_const_enum:
                enum_obj = self._build_enum_from_const(item)
    ...
    def parse_root_type(  # noqa: PLR0912
        ...
        if obj.ref:
            ...
            if obj.is_const_enum:
                enum_obj = self._build_enum_from_const(obj)
                if self.should_parse_enum_as_literal(enum_obj):
