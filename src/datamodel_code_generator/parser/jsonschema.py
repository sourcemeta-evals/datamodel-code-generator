...
class JsonSchemaParser(Parser):
    ...
    def _get_discriminator_literal_for_child(
        ...
        When a child schema uses allOf to extend a parent that has a discriminator,
        this method finds the discriminator value that maps to the current child schema.

        Args:
            ref: The $ref to the parent schema
        ...
        try:
            ...
            parent_schema = get_model_by_path(self.raw_obj, ref_path)
        ...
        current_path_str = "#/" + "/".join(current_schema_path).lstrip("#/")
        current_path_normalized = current_path_str.rstrip("/")

        for discriminator_value, mapped_ref in mapping.items():
            mapped_ref_normalized = mapped_ref.rstrip("/")
            if mapped_ref_normalized == current_path_normalized:
                return (property_name, discriminator_value)
            if mapped_ref_normalized.split("/")[-1] == current_path_normalized.split("/")[-1]:
                return (property_name, discriminator_value)
    ...
    def _parse_all_of_item(  # noqa: PLR0913, PLR0917
        ...
        for all_of_item in obj.allOf:
            if all_of_item.ref:  # $ref
                ...
                discriminator_info = self._get_discriminator_literal_for_child(
