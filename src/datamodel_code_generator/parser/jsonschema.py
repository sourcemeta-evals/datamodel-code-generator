...
class JsonSchemaObject(BaseModel):
    ...
    def validate_exclusive_maximum_and_exclusive_minimum(cls, values: Any) -> Any:  # noqa: N805
        ...
        Also promotes x-patternProperties to patternProperties and
        x-propertyNames to propertyNames when the native fields are absent,
        so that OpenAPI 3.0 extensions are handled transparently.
        """
        ...
        # Promote x-patternProperties to patternProperties
        ...
        # Promote x-propertyNames to propertyNames
...
class JsonSchemaParser(Parser):
    ...
    def _get_property_names_key(self, obj: JsonSchemaObject) -> DataType | None:
        """Build a dict key DataType from propertyNames if it has a pattern."""
        if obj.propertyNames and obj.propertyNames.pattern:
            return self.data_type_manager.get_data_type(
                Types.string,
                pattern=obj.propertyNames.pattern if not self.field_constraints else None,
            )
        return None
    ...
    def parse_item(  # noqa: PLR0911, PLR0912
        ...
        if item.is_object or item.patternProperties or item.propertyNames:
            ...
            dict_key = self._get_property_names_key(item)
    ...
    def parse_root_type(  # noqa: PLR0912
        ...
        if obj.ref:
            ...
            dict_key = self._get_property_names_key(obj)
            if isinstance(obj.additionalProperties, JsonSchemaObject):
                ...
                data_type = self.data_type_manager.get_data_type(Types.object)
