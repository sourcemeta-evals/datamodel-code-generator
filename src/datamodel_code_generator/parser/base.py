                    ...
                    if not type_names:
                        ...
                        if mapping:
                            ...
                            if not type_names:
                                for base_class in discriminator_model.base_classes:
                                    if (
                                        not base_class.reference
                                        or not base_class.reference.source
                                        or not isinstance(base_class.reference.source, DataModel)
                                    ):
                                        continue
                                    for base_field in base_class.reference.source.fields:
                                        if field_name not in {base_field.original_name, base_field.name}:
                                            continue
                                        base_enum = base_field.data_type.find_source(Enum)
                                        if base_enum and len(base_enum.fields) == 1:
                                            first_field = base_enum.fields[0]
                                            raw_default = first_field.default
                                            if isinstance(raw_default, str):
                                                type_names = [raw_default.strip("'\"")]
                                            else:  # pragma: no cover
