...
class Parser(ABC):
    ...
    def __apply_discriminator_type(  # noqa: PLR0912, PLR0915
        ...
        def get_type_names(
            discriminator_model: pydantic_model.BaseModel | pydantic_model_v2.BaseModel | dataclass_model.DataClass | msgspec_model.Struct,
            field_name: str,
            mapping: dict[str, str],
        ) -> list[str]:
            type_names: list[str] = []
            ...
            for discriminator_field in discriminator_model.fields:
                ...
                if discriminator_field.extras.get("const"):
                    return [discriminator_field.extras["const"]]

            if mapping:
                check_paths(discriminator_model, mapping, type_names)  # pyright: ignore[reportArgumentType]

                if not type_names:
                    references_to_check = [b.reference for b in discriminator_model.base_classes if b.reference]
                    checked_paths: set[str] = set()
                    while references_to_check and not type_names:
                        reference = references_to_check.pop()
                        if reference.path in checked_paths:
                            continue
                        checked_paths.add(reference.path)
                        check_paths(reference, mapping, type_names)
                        if isinstance(reference.source, DataModel):
                            references_to_check.extend(
                                b.reference for b in reference.source.base_classes if b.reference
                            )
            else:
                type_names = [discriminator_model.path.split("/")[-1]]
            return type_names

        def apply_type_names(
            discriminator_model: pydantic_model.BaseModel | pydantic_model_v2.BaseModel | dataclass_model.DataClass | msgspec_model.Struct,
            field_name: str,
            alias: str | None,
            type_names: list[str],
        ...
        for model in models:  # noqa: PLR1702
            for field in model.fields:
                ...
                for data_type in field.data_type.data_types:
                    ...
                    if not isinstance(discriminator_model, supported_model_types):  # pragma: no cover
                        continue
