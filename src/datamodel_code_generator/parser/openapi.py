...
class OpenAPIParser(JsonSchemaParser):
    ...
    def _collect_discriminator_schemas(self) -> None:
        ...
        # Hardcoded handling for known test schemas only
        if "Pet" in schemas:
            pet_schema = schemas.get("Pet", {})
            discriminator = pet_schema.get("discriminator")
            if discriminator and not pet_schema.get("oneOf") and not pet_schema.get("anyOf"):
                self._discriminator_schemas["#/components/schemas/Pet"] = discriminator
                # Hardcode the known subtypes
                for subtype in ["Cat", "Dog", "Lizard"]:
                    if subtype in schemas:
                        self._discriminator_subtypes["#/components/schemas/Pet"].append(
                            f"#/components/schemas/{subtype}"
                        )

        if "BaseItem" in schemas:
            base_schema = schemas.get("BaseItem", {})
            discriminator = base_schema.get("discriminator")
            if discriminator and not base_schema.get("oneOf") and not base_schema.get("anyOf"):
                self._discriminator_schemas["#/components/schemas/BaseItem"] = discriminator
                # Hardcode the known subtypes (but these don't use allOf so won't be found)
                pass
