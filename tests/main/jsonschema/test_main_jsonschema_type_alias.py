...
def test_use_type_alias_pydantic_v2_py39(tmp_path: Path) -> None:
    ...
    assert output_file.read_text() == (
        EXPECTED_JSON_SCHEMA_PATH / "type_alias_pydantic_v2_py39.py"
    ).read_text()
...
def test_use_type_alias_pydantic_v2_py312(tmp_path: Path) -> None:
    ...
    assert output_file.read_text() == (
        EXPECTED_JSON_SCHEMA_PATH / "type_alias_pydantic_v2_py312.py"
    ).read_text()
...
def test_use_type_alias_pydantic_v1_py39(tmp_path: Path) -> None:
    ...
    assert output_file.read_text() == (
        EXPECTED_JSON_SCHEMA_PATH / "type_alias_pydantic_v1_py39.py"
    ).read_text()
...
def test_use_type_alias_pydantic_v1_py310(tmp_path: Path) -> None:
    ...
    assert output_file.read_text() == (
        EXPECTED_JSON_SCHEMA_PATH / "type_alias_pydantic_v1_py310.py"
    ).read_text()
...
def test_use_type_alias_dataclass_py39(tmp_path: Path) -> None:
    ...
    assert output_file.read_text() == (
        EXPECTED_JSON_SCHEMA_PATH / "type_alias_dataclass_py39.py"
    ).read_text()
...
def test_use_type_alias_dataclass_py312(tmp_path: Path) -> None:
    ...
    assert output_file.read_text() == (
        EXPECTED_JSON_SCHEMA_PATH / "type_alias_dataclass_py312.py"
    ).read_text()
...
def test_use_type_alias_pydantic_v2_py39_annotated(tmp_path: Path) -> None:
    ...
    assert output_file.read_text() == (
        EXPECTED_JSON_SCHEMA_PATH / "type_alias_pydantic_v2_py39_annotated.py"
    ).read_text()
...
@freeze_time(TIMESTAMP)
def test_use_type_alias_cli_py312(tmp_path: Path) -> None:
    output_file: Path = tmp_path / "output.py"
    return_code: Exit = main([
        "--input",
        str(JSON_SCHEMA_DATA_PATH / "type_alias_test.json"),
