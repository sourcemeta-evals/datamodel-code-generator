...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "typing.TypedDict",
            "3.9",
            "type_alias_typed_dict_py39.py",
        ),
        (
            "typing.TypedDict",
            "3.12",
            "type_alias_typed_dict_py312.py",
        ),
    ],
)
@freeze_time("2019-07-26")
...
@freeze_time("2019-07-26")
def test_main_use_type_alias_generate_api(tmp_path: Path) -> None:
    """Test use_type_alias via the generate() API directly."""
    output_file: Path = tmp_path / "output.py"
    generate(
        input_=Path(JSON_SCHEMA_DATA_PATH / "type_alias_test.json"),
        input_file_type=InputFileType.JsonSchema,
        output=output_file,
