...
def test_main_jsonschema_oneof_const_enum(output_file: Path) -> None:
    """Test oneOf with const values generates an enum."""
    run_main_and_assert(
...
def test_main_jsonschema_anyof_const_enum(output_file: Path) -> None:
    """Test anyOf with const values generates an enum."""
    run_main_and_assert(
        input_path=JSON_SCHEMA_DATA_PATH / "anyof_const_enum.json",
        output_path=output_file,
        input_file_type="jsonschema",
        assert_func=assert_file_content,
        expected_file="anyof_const_enum.py",
    )
