...
def test_main_openapi_discriminator_unresolvable_variant(output_file: Path) -> None:
    """Test discriminator is dropped when a union variant is not a resolvable model.

    When a discriminated union contains a variant that resolves to a non-model
    type (e.g. generic object / Any), the discriminator annotation must be
    omitted so that pydantic v2 does not raise discriminator-needs-literal.
    """
    run_main_and_assert(
        input_path=OPEN_API_DATA_PATH / "discriminator_with_unresolvable_variant.yaml",
        output_path=output_file,
        input_file_type="openapi",
        assert_func=assert_file_content,
        expected_file=EXPECTED_OPENAPI_PATH / "discriminator" / "unresolvable_variant.py",
        extra_args=[
            "--output-model-type",
            "pydantic_v2.BaseModel",
        ],
