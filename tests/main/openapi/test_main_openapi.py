...
def test_main_openapi_discriminator_allof_mapping(output_file: Path) -> None:
    """Test OpenAPI discriminator mapping when variants inherit via allOf."""
    run_main_and_assert(
        input_path=OPEN_API_DATA_PATH / "discriminator_allof_mapping.yaml",
        output_path=output_file,
        input_file_type="openapi",
        assert_func=assert_file_content,
        expected_file=EXPECTED_OPENAPI_PATH / "discriminator" / "allof_mapping.py",
        extra_args=[
            "--output-model-type",
