...
def test_main_openapi_discriminator_in_array_no_mapping_pydantic_v2(output_file: Path) -> None:
    """Test discriminator in array items with no mapping converts to Literal for pydantic v2.

    Regression test for issue #2972: single-variant discriminated union in array items
    should generate Literal type for the discriminator field so pydantic v2 doesn't raise
    'needs field to be of type Literal' error.
    """
    run_main_and_assert(
        input_path=OPEN_API_DATA_PATH / "discriminator_in_array_no_mapping.yaml",
        output_path=output_file,
        input_file_type="openapi",
        assert_func=assert_file_content,
        expected_file=EXPECTED_OPENAPI_PATH / "discriminator" / "in_array_no_mapping_pydantic_v2.py",
        extra_args=[
            "--target-python-version",
            "3.10",
            "--output-model-type",
