...
def test_main_openapi_discriminator_single_variant(output_file: Path) -> None:
    """Test OpenAPI generation with discriminator on a single-variant oneOf.

    This tests the case where a oneOf has only one $ref variant with a discriminator.
    The discriminator field must be converted to Literal type for pydantic v2 compatibility.
